...
const TINFL_STATUS_CANCELLED: i32 = -5;
...
pub enum TINFLStatus {
    /// Decompression was cancelled early by the cancel check.
    ///
    /// The data decompressed so far is valid.
    Cancelled = TINFL_STATUS_CANCELLED as i8,
...
impl TINFLStatus {
    pub fn from_i32(value: i32) -> Option<TINFLStatus> {
        ...
        match value {
            TINFL_STATUS_CANCELLED => Some(Cancelled),
...
impl alloc::fmt::Display for DecompressError {
    ...
    fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
        f.write_str(match self.status {
            TINFLStatus::Cancelled => "Cancelled by the cancel check",
...
/// polling `cancel` at each deflate block boundary and roughly every 16 KiB of
/// decompressed output.
///
/// If `cancel` signals cancellation, decompression ends early and the returned
/// [`DecompressError`] has the status [`TINFLStatus::Cancelled`] and holds the
/// data decompressed so far.
///
/// [`NeverCancel`][crate::cancel::NeverCancel] installs no callback and takes
/// the same code path as [`decompress_to_vec_with_limit`].
#[inline]
#[cfg(feature = "with-alloc")]
pub fn decompress_to_vec_with_cancel(
    input: &[u8],
    zlib_header: bool,
    max_size: usize,
    cancel: &impl crate::cancel::CancelCheck,
) -> Result<Vec<u8>, DecompressError> {
    ...
    let callback;
    let cancel = if cancel.may_cancel() {
        callback = || cancel.is_cancelled();
        Some(&callback as &dyn Fn() -> bool)
    } else {
        None
    };
    decompress_to_vec_inner(input, flags, max_size, cancel)
}
...
mod test {
    ...
    mod cancel {
        ...
        use crate::inflate::{decompress_to_vec_with_cancel, TINFLStatus};
        ...
        #[test]
        fn never_cancelling_matches_plain_decompress() {
            let (raw, compressed) = payload();
            assert_eq!(
                decompress_to_vec_with_cancel(&compressed, true, usize::MAX, &NeverCancel).unwrap(),
                raw
            );
            assert_eq!(
                decompress_to_vec_with_cancel(
                    &compressed,
                    true,
                    usize::MAX,
        ...
        #[test]
        fn cancelling_returns_cancelled_status() {
            let (raw, compressed) = payload();
            for checks_allowed in [0, 2] {
                let err = decompress_to_vec_with_cancel(
                    &compressed,
                    true,
                    usize::MAX,
                    &cancel_after(checks_allowed),
                )
                .unwrap_err();
                assert_eq!(err.status, TINFLStatus::Cancelled);
